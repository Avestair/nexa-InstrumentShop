...
type Product = {
  ...
  description: string;
  price: number;
  category: "Guitar" | "Piano" | "Drums" | "Violin" | "Other";
  brand: string;
  stock: number;
  images: string[];
};
...
async function fetchProduct(slug: string): Promise<Product> {
  // In production, fetch from your API with proper error handling
  return {
    _id: slug,
    name: "Fender Stratocaster Electric Guitar",
    description:
      "Professional-grade American-made Fender Stratocaster with maple neck and alder body. Features three single-coil pickups for iconic tone.",
    price: 129900,
    category: "Guitar",
    brand: "Fender",
    stock: 15,
...
export default function ProductPage({ params }: ProductPageProps) {
  ...
  const product = use(fetchProduct(slug.slug));
  ...
  return (
    <>
      {/* SEO Meta Tags */}
      <Head>
        <title>{`${product.name} | ${product.brand} | Music Store`}</title>
        ...
        <meta
          ...
          content={(product.price / 100).toString()}
        />
      ...
      <div className="min-h-screen bg-white text-black">
        ...
        <main className="container mx-auto px-4 py-6 md:py-12">
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            {/* Product Images - Mobile first approach */}
            <div className="size-[100px] md:sticky md:top-4 md:size-[300px] lg:size-[380px]">
              <Gallery
            ...
            <div className="space-y-4 md:space-y-6">
              <div>
                ...
                <Link
                  ...
                  className="mt-2 inline-block rounded bg-gray-100 px-2 py-1 text-xs transition-colors hover:bg-gray-200"
                  aria-label={`Browse more ${product.category}s`}
                >
                  {product.category}
                </Link>
              </div>
              ...
              <div
                ...
                className="flex gap-3 md:text-2xl"
              >
              ...
              {/* Product description with proper semantics */}
              ...
              {/* Stock Information with visual and text indicators */}
              ...
              {/* Product Actions - Ensure this is accessible */}
              <ProductActions product={product} />
              {/* Additional Info with semantic markup */}
              <section
              ...
              {/* //TODO FIX THE ACTIONS */}
              <div className="fixed right-0 bottom-0 left-0 bg-white p-4 shadow-lg md:hidden">
                <ProductActions product={product} isMobile />
              </div>
            </div>
          </div>
        </main>
